...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#need <- c("foreign", "sandwich", "ggplot2", "stargazer") # list packages you need}
\CommentTok{#have <- need %in% rownames(installed.packages()) # see which are already}
\CommentTok{#if(any(!have)) install.packages(need[!have]) # installs the missing ones }
\CommentTok{#invisible(lapply(need, library, character.only=T)) # then loads them all}
